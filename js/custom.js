...
/*----------------------------------------------------*/
	...
	*/
	...
	jQuery('#maps').gMap({
		address: jQuery('#maps').attr('data-address'),
		zoom: 12,
		markers:[
			{
				address: jQuery('#maps').attr('data-address'),
				html: jQuery('#maps').attr('data-address-description'),
				popup: true
			}
		]
	});
...
$("form").submit(function (e) {
	...
	var phone = thisForm.find("#phone").val();
    var text = thisForm.find("#text").val();
    ...
    if (isValidEmail(email) && (text.length > 1) && (name.length > 1)) {
    	thisForm.find('label').hide();
    	
    	ldkTrackContact($(this), function() {
        	btn.fadeOut(400, function() {
        		btn.replaceWith('<div class="alert alert-success" style="margin: 0 15px 15px 15px; text-align: center;"><strong>Muchas gracias, recibimos tu contacto!</strong></div>');
        	});
    	});
    /* } else {
...
function goToByScroll(id){
	jQuery('html, body').animate({scrollTop: jQuery("#"+id).offset().top - 200}, 'slow');
}
